...
- **Scheduled work**: two `node-cron` jobs (`grep -n "cron.schedule" server.js`). `scanPrices` (every 6h) refreshes watchlist/portfolio prices and fires threshold alerts — the app's main freshness mechanism, not on-demand polling. `emailDailyReport` (`30 12 * * 1-5`, pinned to UTC) fetches the `reports` branch read-only and emails that day's `business/daily/*.md` via `sendEmail`; it is the only place the app shells out to `git` at runtime, and it must stay read-only because the droplet's checkout is a deploy target.
